...
pub struct Behaviour<B: BlockT> {
	...
	/// [`request_responses::RequestResponsesBehaviour`].
	#[behaviour(ignore)]
	data_request_protocol_name: String,
}
...
impl<B: BlockT> Behaviour<B> {
	...
	pub fn new(
		...
		data_request_protocol_config: request_responses::ProtocolConfig,
		...
		let data_request_protocol_name = data_request_protocol_config.name.to_string();
		...
		request_response_protocols.push(data_request_protocol_config);
...
impl<B: BlockT> NetworkBehaviourEventProcess<CustomMessageOutcome<B>> for Behaviour<B> {
	fn inject_event(&mut self, event: CustomMessageOutcome<B>) {
		match event {
			...
			CustomMessageOutcome::DataRequest { target, request, pending_response } => {
				let mut buf = Vec::with_capacity(request.encoded_len());
				if let Err(err) = request.encode(&mut buf) {
					log::warn!(
						target: "sync",
						"Failed to encode data request {:?}: {:?}",
						request, err
					);
					return
				}
				...
				self.request_responses.send_request(
					&target,
					&self.data_request_protocol_name,
					buf,
					pending_response,
					IfDisconnected::ImmediateError,
				);
