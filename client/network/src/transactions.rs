...
impl TransactionsHandlerPrototype {
	...
	pub fn build<B: BlockT + 'static, H: ExHashT>(
		...
		new_transaction_sender: Option<mpsc::UnboundedSender<(PeerId, B::Extrinsic)>>
			...
			for t in transactions {
				...
				match self.pending_transactions_peers.entry(hash.clone()) {
					Entry::Vacant(entry) => {
						...
						if let Some(ref tx_sender) = self.new_transaction_sender {
							debug!(
								target: "sync",
								"Sending new transaction for data sync processing: {:?}", t.encode(),
							);
							let _ = tx_sender.unbounded_send((who, t));
						}
