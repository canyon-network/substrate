...
pub struct DataRequestHandler<B: BlockT> {
...
impl<B: BlockT> DataRequestHandler<B> {
	...
	fn handle_request(
		...
		log::trace!(
			target: LOG_TARGET,
			"Handling data request from {}: Block {:?}, Starting at {:?}, no_proof={}",
			peer,
			request.block,
		...
		let result = if reputation_changes.is_empty() {
			// TODO: fill in the real response data.
			...
			log::trace!(
				target: LOG_TARGET,
				"DataResponse contains {} keys, {}, proof nodes, complete={}, from {:?} to {:?}",
